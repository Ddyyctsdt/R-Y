...
Reverted to stable split logic (no MP4Box), fixed Zip creation with subprocess and fsync,
    ...
    try:
        ...
        while start < duration:
            ...
            for retry in range(5):
                ...
                if end - start < 0.5:
                    logger.error(f"Chunk {idx} cannot be made small enough – using binary split on this segment")
                    # binary split on the original video segment? Actually we can't binary split a segment without re-extracting; as last resort we'll accept oversized and later resplit in send_video_parts
                    # Instead, we'll abort splitting and return unsplit whole file; this will trigger send_video_parts binary fallback later.
                    return [video_path]
            else:
                logger.warning(f"Chunk {idx} still oversized after 5 retries – using binary fallback later")
                parts.append(out_path)
            start = end
            idx += 1
        return parts
        ...
        return [video_path]  # unsplit fallback
...
def resplit_video_chunk(chunk_path: str, max_size_bytes: int = MAX_SEND_SIZE) -> List[str]:
    ...
    try:
        dur = subprocess.run(
            [ffprobe, "-v", "error", "-show_entries", "format=duration", "-of", "csv=p=0", chunk_path],
            capture_output=True, text=True, check=True
        )
        ...
        start = 0.0
        idx = 1
        out_dir = os.path.dirname(chunk_path)
        parts = []
        while start < duration:
            end = min(start + chunk_time, duration)
            out_path = os.path.join(out_dir, f"resplit_{idx:03d}.mp4")
            cmd = [
                ffmpeg, "-y", "-ss", str(start), "-to", str(end),
                "-i", chunk_path,
                "-fs", str(max_size_bytes),
        ...
        logger.warning(f"resplit_video_chunk failed: {e}")
        return [chunk_path]
        ...
        for attempt in range(1, 4):
            ...
            try:
                if video_mode == "showable" and send_mode == "playable":
                    ...
                    # For document mode, send as document (even if mp4)
...
def handle_download(chat_id: int, video_id: str, index: int, confirmed: bool = False) -> None:
    ...
    try:
        ...
        if send_mode == "playable":
            ...
            # ZIP mode with subprocess and classic naming
            zip_base = os.path.splitext(video_path)[0]
            zip_path = f"{zip_base}.zip"
            try:
                subprocess.run(["zip", "-r", zip_path, os.path.basename(video_path)],
                               cwd=os.path.dirname(video_path), check=True)
                # Flush to disk
                time.sleep(0.5)
                try:
                ...
                safe_remove(video_path)
                return
            # Binary split with classic naming: prefix=basename_without_ext, ext=".zip"
            parts = split_file_binary(zip_path, prefix=os.path.splitext(os.path.basename(zip_path))[0], ext=".zip")
            safe_remove(zip_path)

        # Oversized part handling (emergency resplit)
        fixed_parts = []
        for p in parts:
            if os.path.getsize(p) > MAX_SEND_SIZE and send_mode == "playable":
                logger.warning(f"Part {os.path.basename(p)} oversized – attempting resplit")
                sub_parts = resplit_video_chunk(p, MAX_SEND_SIZE)
                if sub_parts and sub_parts != [p]:
                    fixed_parts.extend(sub_parts)
                    safe_remove(p)
                else:
                    # last resort binary split (with warning)
                    logger.warning("Resplit failed, using binary split on oversized chunk")
                    binary_parts = split_file_binary(p)
                    fixed_parts.extend(binary_parts)
                    safe_remove(p)
            else:
